export function githubRepoParser(rawJson: string) {
    ...
    const parsedRepos = githubRepos.entries.map((repo: { url: string; name: string; lang: string; desc: string; info: string; }) => ({
...
export async function relevantReposParser(repos: ReturnType<typeof githubRepoParser>, jobDesc: string) {
    ...
    const validatedRepos = repos.map((repo: { url: string; name: string; lang: string; desc: string; info: string; }) => ({
        ...repo,
        url: repo.url || '',
        name: repo.name || 'Unknown',
        lang: repo.lang || 'Unknown',
        desc: repo.desc || '',
        info: repo.info || '',
...
export function rawToExperience(rawJson: string) {
    ...
    const parsedExperience = rawExperience[0].experience.map((exp: { title: string; company: string; location: string; duration: string; employment_type: string; description: string; }) => ({
        title: exp.title,
        company: exp.company,
...
export function extractProfileInfo(rawJson: string) {
    ...
    return {
        ...
        skills: profile.experience
            .filter((exp: { skills: string[] }) => exp.skills && exp.skills.length > 0)
            .flatMap((exp: { skills: string[] }) => exp.skills),
        certifications: profile.certifications.map((cert: { name: string; issuer: string; issued_date: string; }) => ({
            name: cert.name,
            issuer: cert.issuer,
